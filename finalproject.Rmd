...
formula = as.formula(paste("price", "~", paste(tmp, collapse=" + ")))
...
plot(x=1:6,reg.sum$bic,xlab="number of predictors",ylab="BIC",type="b");points(which.min(reg.sum$bic),reg.sum$bic[which.min(reg.sum$bic)],col ="red",cex=2,pch=20)

plot(x=1:6,reg.sum$adjr2,xlab="number of predictors",ylab="Adjusted R2",type="b");points(which.max(reg.sum$adjr2),reg.sum$adjr2[which.max(reg.sum$adjr2)],col ="red",cex=2,pch=20)

plot(x=1:6,reg.sum$rsq,xlab="number of predictors",ylab="R2",type="b");points(which.max(reg.sum$rsq),reg.sum$rsq[which.max(reg.sum$rsq)],col ="red",cex=2,pch=20)
```
...
for(j in 1:k) {
  best.fit=leaps::regsubsets(price~., data=real[folds!=j,], nvmax = 6)
  for(i in 1:6) {
...
reg.best = leaps::regsubsets(price ~ . ,data=real, nvmax = 6)
coef(reg.best, 5)
```
...
fit_log = lm(log(price) ~., real[,-c(1,7)])
summary(fit_log)
vif(fit_log)
```
...
shapiro.test(fit_log$residuals) # H0: Errors are normally distributed: Since p-value is less than 0.05, it is significant to reject the null hypothesis that the errors are normally distributed. Hence the assumption of normality is violated.
durbinWatsonTest(fit_log) # H0:Errors are uncorrelated: Since p-value is greater than 0.05, cannot reject H0, which means errors are uncorrelated, hence the uncorrelation assumption is not violated.
ncvTest(fit_log) # H0:Errors have a constant vairance: Since p-value is less than 0.05, it is significant to reject the H0 and which means the constant variance assumption is violated.
```
...
##Scale the data
standardized_real <- cbind(scale(real[,-8]),real[8])

## split into predictors and outcome train and test data
test_data <- standardized_real[-train_ind, ]
train_data <- standardized_real[train_ind, ]
...
```{r}
train$price_c <- as.factor(as.character(train$price_c))
test$price_c <- as.factor(as.character(test$price_c))
x_train <- model.matrix(price_c~., train)[,-1]
y_train <- train$price_c
x_test <- model.matrix(price_c~., test)[,-1]
y_test <- test$price_c
...
pred.error.ela <- error.ela.a[which.min(error.ela),1]
```
...
CV.errors <- function(CVdata, k, m){
  ...
  for(m in 1:M){
    ...
    for(i in k_values){
        K <- i
        cv_error_df[i, m] <- KNN.error(cv_tr, cv_tst, K)
        }
    }
  
  rownames(cv_error_df) <- paste0("k=", k_values)
  names(cv_error_df) <- paste("fold", seq(1:m))
  cv_error_df$"CV error" <- rowMeans(cv_error_df)
  round(cv_error_df, digits=3)
}

k <- CV.errors(train, k=10, m=10)

pred.error.knn <- round(KNN.error(train, test, k=7), digits=3)
```
...
```{r}
pe <- as.data.frame(c(round(pred.error.lr,3),  
                      round(pred.error.ela,3)))

rownames(pe) <- c("Logistic Regression",
                  "Elastic Net")
colnames(pe) <- "Prediction Error"

knitr::kable(pe, align = "c")
```
...
###AIC method
model1 = train(price ~ date + age + MRT + store + latitude + longitude + age:longitude + MRT:store +
               MRT:latitude + store:latitude, data = real, method = "lm", trControl = train.control)

###BIC method
model2 = train(price ~ date + age + MRT + store + latitude + MRT:store + MRT:latitude + store:latitude,
               data = real, method = "lm", trControl = train.control)

###regsubsets method
model3 = train(price ~ date + age + MRT + store + latitude + age:latitude + MRT:store + MRT:latitude +
                 store:latitude, data = real, method = "lm", trControl = train.control)
...
```{r}
final.fit = lm(price ~ date + age + MRT + store + latitude + MRT:store + MRT:latitude + store:latitude,
               data = real)
summary(final.fit)
par(mfrow = c(2,2))
