...
```{r}
corp = ggcorr(real, label = T)
pairp = ggpairs(real, lower = list(continuous = wrap("smooth_loess", alpha = 1, size = 1, color='#00BFC4')), upper = list(continuous = wrap("cor", alpha = 1, size = 3))) +
...
model_1 = train(price ~ .-longitude, data = real, method = "lm", trControl = train.control )
print(model_1)
model_2 = train(price ~ ., data = real, method = "lm", trControl = train.control )
print(model_2)
```
...
```{r}
price_c = cut(real$price, breaks = c(7.49, 35.1, 62.6, 118), labels = c("low","median","high"))
